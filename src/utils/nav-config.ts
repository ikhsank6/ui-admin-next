...
export const allMenuItems: MenuItem[] = [
  {
    ...
    href: '/dashboard',
    roles: ['pemohon', 'staff', 'koordinator', 'kasubdit', 'direktur', 'superadmin', 'balai', 'sesdit', 'menteri', 'dirjen', 'sekjen', 'hukum'],
  },
  {
    id: 'permohonan',
    label: 'Permohonan',
...
export const navSections: NavSection[] = [
  {
    ...
    icon: LayoutDashboard,
    items: [
      menuToNav(allMenuItems.find((m) => m.id === 'dashboard')!),
    ],
  },
  {
    title: "Permohonan",
  ...
  },
  {
    ...
    icon: Settings,
    items: [
      ...(allMenuItems.find((m) => m.id === 'master')!.children ?? []).map(menuToNav),
    ],
  },
  {
    title: "Profil",
    icon: User,
    items: [
      ...(allMenuItems.find((m) => m.id === 'profil')!.children ?? []).map(menuToNav),
    ],
  },
  {
    title: "Laporan",
    icon: BarChart3,
    items: [
      ...(allMenuItems.find((m) => m.id === 'laporan')!.children ?? []).map(menuToNav),
    ],
  },
  {
